...
class PandaEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if np.any(global_target_error > error_thresh):           
            ...
            string_debug = f"Target reached {self.planning._points[2]} on {now.day}/{now.month}/{now.year} at {now.hour}:{now.minute}:{now.second}"                                 
            print(string_debug)  
        ...
        err_x = np.abs(np.linalg.norm(X_target) - np.linalg.norm(delta_x))
        delta_y = np.asarray(Y_target.reshape(3,1) - Y.reshape(3,1))
        err_y = np.abs(np.linalg.norm(Y_target) - np.linalg.norm(delta_y))
        delta_z = np.asarray(Z_target.reshape(3,1) - Z.reshape(3,1))
        err_z = np.abs(np.linalg.norm(Y_target) - np.linalg.norm(delta_z))
                
        lambda_err = 1
        lambda_eff = 0.001        
        delta_x_error = err_x + err_y + err_z
        total_target = np.linalg.norm(X_target) + np.linalg.norm(Y_target) + np.linalg.norm(Z_target)
        acc_error = np.linalg.norm(avg_acc_values)
        #reward = np.exp(-lambda_err*np.square(delta_x_error)) - np.clip(lambda_eff*acc_error, 0, 1)
        reward = lambda_err*(delta_x_error) #- np.clip(lambda_eff*acc_error, 0, 1)
        info = {} 
...
if __name__ == '__main__':
    ...
    callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=1000, verbose = 3)
