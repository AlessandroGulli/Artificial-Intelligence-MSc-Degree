...
class BulletRobotMultiprocessing(object):
    def __init__(self, description_path, uid=None, config=None, realtime_sim=False):
        ...
        if uid is None:                                         
            pb = bc.BulletClient(connection_mode=pybullet.GUI)    
            pb.setAdditionalSearchPath(pybullet_data.getDataPath())          
            uid = pb._client
        else:
            pb = bc.BulletClient(connection_mode=pybullet.DIRECT)    
            pb.setAdditionalSearchPath(pybullet_data.getDataPath())          
            uid = pb._client       
        ...
        self._uid = uid
        self.pb = pb
        self.pb.resetSimulation(physicsClientId=self._uid)
        ...
        if extension == "urdf":
            robot_id = self.pb.loadURDF(
                description_path, useFixedBase=True, physicsClientId=self._uid)
        elif extension == 'sdf':
            robot_id = self.pb.loadSDF(
                description_path, useFixedBase=True, physicsClientId=self._uid)
        elif extension == 'bullet':
            robot_id = self.pb.loadBullet(
                description_path, useFixedBase=True, physicsClientId=self._uid)
        else:
            robot_id = self.pb.loadMJCF(
                description_path, useFixedBase=True, physicsClientId=self._uid)

        self._id = robot_id

        self.pb.setGravity(0.0, 0.0, -9.81, physicsClientId=self._uid)       
        
        self._dof = self.pb.getNumJoints(self._id)                 
        
        '''
        joint_states = pb.getJointStates(self._id, self.range(self._all_joints))
        joint_pos = [state[0] for state in joint_states]
        joint_vel = [state[1] for state in joint_states]
        joint_acc = [0. for _ in range(self.dof)]
        
        print(joint_pos,joint_vel,joint_acc)
        
        self._gravity_compensation = list(pb.calculateInverseDynamics(self._id, joint_pos, joint_vel, joint_acc))
        
        print(self._gravity_compensation)
        '''
                
        self.pb.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=90, cameraPitch=-30, cameraTargetPosition=[0.55,0,0.2])
        
        if realtime_sim:
            self.pb.setRealTimeSimulation(1, physicsClientId=self._uid)
            self.pb.setTimeStep(0.01, physicsClientId=self._uid)
        self._rt_sim = realtime_sim

        self._all_joints = np.array(
            range(self.pb.getNumJoints(self._id, physicsClientId=self._uid)))
    ...
    def reset(self):
        ...
        extension = self.description_path.split('.')[-1]
        if extension == "urdf":
            robot_id = self.pb.loadURDF(
                self.description_path, useFixedBase=True, physicsClientId=self._uid)
        elif extension == 'sdf':
            robot_id = self.pb.loadSDF(
                self.description_path, useFixedBase=True, physicsClientId=self._uid)
        elif extension == 'bullet':
    ...
    def step_sim(self):
        self.pb.stepSimulation(physicsClientId=self._uid)
    ...
    def set_ft_sensor_at(self, joint_id, enable=True):
        ...
        self.pb.enableJointForceTorqueSensor(self._id, joint_id, enable, physicsClientId=self._uid)
        

    def __del__(self):        
        self.pb.disconnect(physicsClientId=self._uid)

    def _use_last_defined_link(self):
        joint_information = self.pb.getJointInfo(
    ...
    def state(self):
        ...
        tip_state = {}
        ft_joint_state = self.pb.getJointState(self._id, max(
    ...
    def jacobian(self, joint_angles=None):
        ...
        linear_jacobian, angular_jacobian = self.pb.calculateJacobian(bodyUniqueId=self._id,
    ...
    def get_link_state(self, link_idx, as_tuple=False):
        ...
        link_state = self.pb.getLinkState(self._id, link_idx, computeLinkVelocity = 1, physicsClientId=self._uid)
    ...
    def inertia(self, joint_angles=None):
        ...
        inertia_tensor = np.array(self.pb.calculateMassMatrix(
    ...
    def inverse_kinematics(self, position, orientation=None):
        ...
        if orientation is None:
            ...
            solution = self.pb.calculateInverseKinematics(self._id,
                                                     self._ee_link_idx,
    ...
    def set_joint_velocities(self, cmd, joints=None):
        ...
        self.pb.setJointMotorControlArray(
            self._id, joints, controlMode=self.pb.VELOCITY_CONTROL, targetVelocities=cmd, physicsClientId=self._uid)
    ...
    def set_joint_torques(self, cmd, joints=None):
        ...
        self.pb.setJointMotorControlArray(
            self._id, joints, controlMode=self.pb.TORQUE_CONTROL, forces=cmd, physicsClientId=self._uid)
    ...
    def set_joint_positions_delta(self, cmd, joints=None, forces=None):
        ...
        self.pb.setJointMotorControlArray(self._id, joints,
    ...
    def set_joint_positions(self, cmd, joints=None, forces=None):
        ...
        self.pb.setJointMotorControlArray(self._id, joints, controlMode=self.pb.POSITION_CONTROL,
                                     targetPositions=cmd, targetVelocities=vels, physicsClientId=self._uid)
     
    def set_gripper(self, cmd, force):
        """
        Set target gripper positions. Use for position controlling.

        :param cmd: gripper position values
        :type cmd: [float] * self._nu
        :param force: gripper forces values
        :type force: [float] * self._nu

        """
        
        if force > 0:
            self.pb.setJointMotorControl2(self._id, 9, self.pb.POSITION_CONTROL, cmd, force=force)
            self.pb.setJointMotorControl2(self._id, 10, self.pb.POSITION_CONTROL, cmd, force=force)
        else:
            self.pb.setJointMotorControl2(self._id, 9 , self.pb.POSITION_CONTROL,  cmd, maxVelocity = 0.05, physicsClientId=self._uid)            
            self.pb.setJointMotorControl2(self._id, 10, self.pb.POSITION_CONTROL,  cmd, maxVelocity = 0.05, physicsClientId=self._uid)
    ...
    def get_joint_info(self):
        ...
        for idx in self._all_joints:
            info = self.pb.getJointInfo(self._id, idx, physicsClientId=self._uid)
    ...
    def get_joint_state(self, joint_id=None):
        ...
        if joint_id is None:
            ...
            for idx in self._movable_joints:
                joint_state = self.pb.getJointState(
            ...
            jnt_state = self.pb.getJointState(
    ...
    def set_joint_angles(self, joint_angles, joint_indices=None):
        ...
        for i, jnt_idx in enumerate(joint_indices):
            self.pb.resetJointState(self._id, jnt_idx,
    ...
    def get_movable_joints(self):
        ...
        for i in self._all_joints:
            joint_info = self.pb.getJointInfo(
    ...
    def get_joint_limits(self):
        ...
        for k, idx in enumerate(self._movable_joints):
            lower_lim[k] = self.pb.getJointInfo(
                self._id, idx, physicsClientId=self._uid)[8]

            upper_lim[k] = self.pb.getJointInfo(
    ...
    def get_joint_velocity_limits(self):
        vel_lim = np.zeros(self.n_joints())
        for k, idx in enumerate(self._movable_joints):
            vel_lim[k] = self.pb.getJointInfo(self._id, idx, physicsClientId=self._uid)[11]        
        return  vel_lim   
    ...
    def set_ctrl_mode(self, ctrl_type='pos'):
        ...
        for k, jnt_index in enumerate(self._movable_joints):
            self.pb.resetJointState(self._id, jnt_index,
                               angles[k], physicsClientId=self._uid)

        if ctrl_type == 'pos':

            self.pb.setJointMotorControlArray(self._id, self._movable_joints, self.pb.POSITION_CONTROL,
                                         targetPosition=angles, forces=[500]*len(self._movable_joints), physicsClientId=self._uid)

        else:
            self.pb.setJointMotorControlArray(self._id, self._movable_joints, self.pb.VELOCITY_CONTROL,
                                         forces=[0.0]*len(self._movable_joints), physicsClientId=self._uid)

    def get_torque_compensation(self):
    
        joint_states = self.pb.getJointStates(self._id, range(self._dof - 2))
        joint_pos = [state[0] for state in joint_states]
        joint_vel = [state[1] for state in joint_states]
        joint_acc = [0. for _ in range(self._dof - 2) ]        
       
        self._gravity_compensation = list(self.pb.calculateInverseDynamics(self._id, joint_pos, joint_vel, joint_acc))        
        
        return self._gravity_compensation	    	                                         

    def triangle_mesh(self):

        visual_shape_data = self.pb.getVisualShapeData(
        ...
        for i, data in enumerate(visual_shape_data):
            ...
            inv_loc_inertial_pos, inv_loc_inertial_ori = self.pb.invertTransform(loc_inertial_pos,
                                                                            loc_inertial_ori, physicsClientId=self._uid)
            tp, to = self.pb.multiplyTransforms(inv_loc_inertial_pos,
                                           inv_loc_inertial_ori,
                                           pos, ori, physicsClientId=self._uid)

            global_pos, global_ori = self.pb.multiplyTransforms(
