...
class BulletRobot(object):
    def __init__(self, description_path, uid=None, config=None, realtime_sim=False):
        ...
        pb.setGravity(0.0, 0.0, -9.81, physicsClientId=self._uid)       
        
        self._dof = pb.getNumJoints(self._id)                 
        ...
        pb.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=90, cameraPitch=-30, cameraTargetPosition=[0.55,0,0.2])
    ...
    def set_gripper(self, cmd, force):
        ...
        if force > 0:
            pb.setJointMotorControl2(self._id, 9, pb.POSITION_CONTROL, cmd, force=force)
            pb.setJointMotorControl2(self._id, 10, pb.POSITION_CONTROL, cmd, force=force)
        else:
            pb.setJointMotorControl2(self._id, 9 , pb.POSITION_CONTROL,  cmd, maxVelocity = 0.05, physicsClientId=self._uid)            
            pb.setJointMotorControl2(self._id, 10, pb.POSITION_CONTROL,  cmd, maxVelocity = 0.05, physicsClientId=self._uid)
    ...
    def get_joint_velocity_limits(self):
        ...
        for k, idx in enumerate(self._movable_joints):
            vel_lim[k] = pb.getJointInfo(self._id, idx, physicsClientId=self._uid)[11]        
        return  vel_lim   
    ...
    def get_torque_compensation(self):
        ...
        joint_states = pb.getJointStates(self._id, range(self._dof - 2))
        joint_pos = [state[0] for state in joint_states]
        joint_vel = [state[1] for state in joint_states]
