...
import numpy as np
from pybullet_robot.bullet_robot_multiprocessing import BulletRobotMultiprocessing
...
class PandaArmMP(BulletRobotMultiprocessing):
    ...
    def __init__(self, robot_description=description_path, config=ROBOT_CONFIG, uid=None, *args, **kwargs):    
        ...
        self._joint_names.append('panda_finger_joint1')    
        self._joint_names.append('panda_finger_joint2') 

        BulletRobotMultiprocessing.__init__(self, robot_description, uid=uid, config=config, realtime_sim=False,**kwargs)    
    ...
    def gripper_open(self, pos=0.04, force=0):        
        self.set_gripper(pos, force)
    
    def gripper_close(self, pos=0.026, force=100):        
        self.set_gripper(pos, force)
    
    def torque_compensation(self):
    	return self.get_torque_compensation()
