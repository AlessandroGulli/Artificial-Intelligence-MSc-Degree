...
class PandaEnv(gym.Env):
    ...
    def reset(self):
        ...
        # Select from conf coordinates
        if self.roll_out_state['Change_Target'] == True:
            self.roll_out_state['Target']= self.planning._points[self.index_waypoint]            
            self.roll_out_state['Change_Target'] = False 
            self.index_waypoint += self.inc_dec 
            string_debug = f"Target changed: {self.roll_out_state['Target']}"
            print(string_debug)
            if self.index_waypoint > len(self.planning._points):
               self.self.inc_dec = -1       	
    ...
    def step(self, action):
        ...
        pb.addUserDebugLine(self.robot.ee_pose()[0], self.roll_out_state['Target'][0:3], lineColorRGB=self.yellow[0:3], lineWidth=2.0, lifeTime=0, physicsClientId=self.robot._uid) 
        ...
        err_x = np.linalg.norm(delta_x) #np.abs(np.linalg.norm(X_target) - np.linalg.norm(delta_x))
        delta_y = np.asarray(Y_target.reshape(3,1) - Y.reshape(3,1))
        err_y = np.linalg.norm(delta_y)#np.abs(np.linalg.norm(Y_target) - np.linalg.norm(delta_y))
        delta_z = np.asarray(Z_target.reshape(3,1) - Z.reshape(3,1))
        err_z = np.linalg.norm(delta_z)#np.abs(np.linalg.norm(Y_target) - np.linalg.norm(delta_z)) 
        ...
        reward = -lambda_err*delta_x_error - lambda_eff*acc_error       
        info = {} 
...
if __name__ == '__main__':
    ...
    # of 2 layers of size 64 each with Tanh activation function
    policy_kwargs = dict(activation_fn=th.nn.Tanh, ortho_init=True, log_std_init=0, net_arch=[dict(pi=[64, 64], vf=[64, 64])])
    ...
    callback = CallbackList([callback_max_episodes, eval_callback_best])
    
    model = PPO("MlpPolicy", env, learning_rate=2e-4, policy_kwargs=policy_kwargs, verbose=3, tensorboard_log="./RL/panda_PPO_tensorboard/").learn(int(1e15), callback=callback, tb_log_name="telemetry")    
