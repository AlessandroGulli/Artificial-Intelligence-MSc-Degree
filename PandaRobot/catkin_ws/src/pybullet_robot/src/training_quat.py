...
class PandaEnv(gym.Env):
    ...
    def __init__(self, render_enable = False, maxSteps = 10000): 
        ...
        # Observation space
        # delta_x -> 7 items, 3 for position and 4 for orientation
        delta_cartesian_pos_ori = 7
    ...
    def reset(self):
        ...
        delta_ori_quat = np.asarray(euler_to_quaternion_raw(pb, delta_ori)).reshape(4, 1)
        delta_x = np.concatenate([delta_pos, delta_ori_quat])
        ...
        observation = np.concatenate([ delta_x.reshape([7,1]), self.goal_joint_angles[0:7].reshape([7,1]), self.goal_joint_velocities[0:7].reshape([7,1]) ]).reshape(-1)         
    ...
    def step(self, action):
        ...
        observation = np.concatenate([ delta_x.reshape([7,1]), self.robot.angles()[0:7].reshape([7,1]), self.robot.joint_velocities()[0:7].reshape([7,1]) ]).reshape(-1) 
        ...
        acc_error = np.linalg.norm(curr_joint_accelerations)
        #reward = -lamba_err*delta_x_error - lamba_eff*acc_error
        reward = np.exp(-lambda_err*np.square(delta_x_error)) - lambda_eff*acc_error
        info = {} 
...
if __name__ == '__main__':
    ...
    policy_kwargs = dict(activation_fn=th.nn.Tanh, net_arch=[dict(pi=[256, 256, 256], vf=[256, 256, 256])])
    ...
    eval_callback_best = CustomEvalCallback(env, best_model_save_path=log_dir, log_path=log_dir, n_eval_episodes=2, eval_freq=1000, deterministic=True, render=False)
    callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=10000, verbose = 3)
