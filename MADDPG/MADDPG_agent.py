...
class MADDPG():
    ...
    #def act(self, all_states, add_noise=True, add_train=True):
    def act(self, all_states, add_noise=True):
        # pass each agent's state from the environment and calculate its action
        all_actions = []
        for agent, state in zip(self.agents, all_states):
            #action = agent.act(state, noise_weight=self.noise_weight, add_noise=self.noise_on, add_train=self.train_on)
    ...
    def load_agents(self, path):
        # load models for local actor and critic of each agent
        for i, agent in enumerate(self.agents):
            agent.actor_local.load_state_dict(torch.load(path + f"checkpoint_actor_agent_{i}.pth"))
            agent.critic_local.load_state_dict(torch.load(path + f"checkpoint_critic_agent_{i}.pth"))        
